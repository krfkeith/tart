...
TEST_F(ParserTest, Types) {
  ...
  ast = parseType("X");
  ASSERT_EQ(ASTNode::Id, ast->getNodeType());
  EXPECT_AST_EQ("X", ast);
  ...
  ast = parseType("X[]");
  ASSERT_EQ(ASTNode::Array, ast->getNodeType());
  ...
  ast = parseType("X.Y");
  ASSERT_EQ(ASTNode::Member, ast->getNodeType());
  EXPECT_AST_EQ("X.Y", ast);

  ast = parseType("bool");
  ASSERT_EQ(ASTNode::BuiltIn, ast->getNodeType());
  ...
  ASSERT_EQ(ASTNode::AnonFn, ast->getNodeType());
...
TEST_F(ParserTest, Terminals) {
  ...
  ASSERT_EQ(ASTNode::LitInt, ast->getNodeType());
  ...
  ASSERT_EQ(ASTNode::LitFloat, ast->getNodeType());
  ...
  ASSERT_EQ(ASTNode::LitChar, ast->getNodeType());
  ASSERT_EQ(uint32_t('c'), dyn_cast<ASTCharLiteral>(ast)->getValue());

  // String literals
  ast = parseExpression("\"c\"");
  ASSERT_EQ(ASTNode::LitString, ast->getNodeType());
  ASSERT_PRED2(ASTCmp, "\"c\"", ast);

  // Boolean literals
  ast = parseExpression("true");
  ASSERT_EQ(ASTNode::LitBool, ast->getNodeType());
...
TEST_F(ParserTest, SimpleExpressions) {
  ...
  ASSERT_EQ(ASTNode::Call, ast->getNodeType());
  ...
  ASSERT_EQ(ASTNode::LogicalAnd, ast->getNodeType());
  EXPECT_AST_EQ("LogicalAnd(1, 1)", ast);

  ast = parseExpression("1 or 1");
  ASSERT_EQ(ASTNode::LogicalOr, ast->getNodeType());
  ...
  ast = parseExpression("1..1");
  ASSERT_EQ(ASTNode::Range, ast->getNodeType());
...
TEST_F(ParserTest, Statements) {
  ...
  ASSERT_EQ(Stmt::Expression, ast->getNodeType());
  EXPECT_AST_EQ("f();", ast);

  ast = parseStatement("a = b;");
  ASSERT_EQ(ASTNode::Expression, ast->getNodeType());
  EXPECT_AST_EQ("Assign(a, b);", ast);

  ast = parseStatement("return b;");
  ASSERT_EQ(ASTNode::Return, ast->getNodeType());
  ...
  ASSERT_EQ(ASTNode::Yield, ast->getNodeType());
  EXPECT_AST_EQ("yield b;", ast);

  ast = parseStatement("throw b;");
  ASSERT_EQ(ASTNode::Throw, ast->getNodeType());
  EXPECT_AST_EQ("throw b;", ast);

  ast = parseStatement("break;");
  ASSERT_EQ(ASTNode::Break, ast->getNodeType());
  EXPECT_AST_EQ("break;", ast);

  ast = parseStatement("continue;");
  ASSERT_EQ(ASTNode::Continue, ast->getNodeType());
  ...
  ASSERT_EQ(ASTNode::While, ast->getNodeType());
...
TEST_F(ParserTest, Decls) {
  ...
  ASSERT_EQ(ASTNode::Template, ast->getNodeType());
  EXPECT_AST_EQ("[%T] def X (x:int, y:int) -> int", ast);

  ast = parseDeclaration("class X[%T] {}");
  ASSERT_EQ(ASTDecl::Template, ast->getNodeType());
