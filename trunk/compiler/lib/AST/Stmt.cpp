...
void BlockStmt::trace() const {
  for (StmtList::const_iterator it = statements.begin();
...
void BlockStmt::format(FormatStream & out) const {
  out << "{";
  for (StmtList::const_iterator it = statements.begin(); it != statements.end();
      ++it) {
    out << *it;
  }
  out << "}";
...
void TryStmt::trace() const {
  Stmt::trace();
  for (StmtList::const_iterator it = catchList.begin(); it != catchList.end();
      ++it) {
    (*it)->mark();
  }
  safeMark(bodySt);
...
// -------------------------------------------------------------------
// Intrinsic
void IntrinsicStmt::format(FormatStream & out) const {
}
